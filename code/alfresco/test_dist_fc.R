...
system.time( load(switch(dataset, ba="baByVeg.RData", fc="fcByVeg.RData", fs="fsByVeg.RData", veg="vegetatedArea.RData", age="vegetationAge.RData")) )# about 5 seconds on Atlas CPU
...
system.time(d %>% filter(Vegetation!="Wetland Tundra" & Scenario %in% c("SRES A1B", "SRES A2") & Year >= yrs.lim[1] & Year <= yrs.lim[2]) %>%
...
}) # about 40 seconds on Atlas CPU

# tertiary data tables (summarizing uncertainty bounds for conditional and marginal distributions of the RV)
# conditional uncertainty
system.time({
d.uc <- ms_uc_table(d)
d.RgS.uc <- ms_uc_table(d.RgS)
d.RgM.uc <- ms_uc_table(d.RgM)
d.uc.cond <- ms_uc_table(d.uc, d.RgS.uc, d.RgM.uc)
# marginal uncertainty
d.mean.uc <- ms_uc_table(d, drop=c("Scenario", "Model"))
d.RgM.mean.uc <- ms_uc_table(d.RgM, drop="Model")
d.RgS.mean.uc <- ms_uc_table(d.RgS, drop="Scenario")
d.R.mean.uc <- ms_uc_table(d.R)
# compound average marginal uncertainty levels
d.uc.mar.compound <- ms_uc_table(d.mean.uc, d.RgM.mean.uc, d.RgS.mean.uc, d.R.mean.uc) # smart bind
