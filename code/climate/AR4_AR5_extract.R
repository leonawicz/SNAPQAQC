...
for(k in 1:2){
	...
	if(k==1){
		...
		save(stats.out, results.years, region.names.out, agg.stat.names, agg.stat.colnames,
			file=paste0("/workspace/UA/mfleonawicz/projects/SNAPQAQC/data/regional/stats/",models[1],"_",models[4],"_annual_regions_stats.RData"))
		...
		for(z in 1:length(arid)){ # 12 combinations
			...
			save(samples.out, samples.names, region.names.out, n.samples, file=paste0("/workspace/UA/mfleonawicz/projects/SNAPQAQC/data/regional/samples/",lab,"_annual_regions_samples.RData"))
		}
	}
	if(k==2 & !is.null(cities)){
		...
		d <- stats.out[[1]]
		save(d, d.cities, results.years, file=paste0("/workspace/UA/mfleonawicz/projects/SNAPQAQC/data/cities/",models[1],"_",models[4],"_annual_cities_batch", cities.batch, "_", domain, ".RData"))
	}
}
