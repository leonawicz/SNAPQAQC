...
load(switch(dataset, ba="baByVeg.RData", fc="fcByVeg.RData", fs="fsByVeg.RData", veg="vegarea.RData", age="vegage.RData"))
...
```{r}
d %>% filter(Model=="CCCMAcgcm31") %>% group_by(Scenario, Year) %>% summarise(Avg=mean(Val))
```
