...
opts_chunk$set(cache=TRUE, eval=TRUE, echo=FALSE, tidy=TRUE, messages=FALSE, warnings=FALSE, dpi=300, fig.height=7, fig.width=10.5)
...
one value (to the nearest 0.1 degrees Celsius) occurring in a clump roughly `r signif(max(d[,TotalArea]), 2)` square km in size.
...
Half of the map is filled in by values occurring in clumps greater than `r d2[RCS <= 0.5, max(Area)]` square km.
